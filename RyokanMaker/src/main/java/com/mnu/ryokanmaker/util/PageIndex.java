...
public class PageIndex {
	...
	public static String pageList(int page,int totpage,String url, int maxlist) {
		return pageList(page, totpage, url, maxlist, "");
	}

	// extraQuery : 페이지를 넘겨도 유지할 추가 조건. "&status=답변대기"처럼 인코딩까지 끝낸 문자열을 넘긴다.
	public static String pageList(int page,int totpage,String url, int maxlist, String extraQuery) {
		//String addtag : 검색추가시
		...
		String extra = (extraQuery == null) ? "" : extraQuery;
		...
		idx_pre = ((page-1) / maxlist);              // Priview 페이지 계산
	  	...
	  	// Prev 표시 부분
	  	if(idx_pre > 0) {
	  		s_pre = "<a class='list' href='"+url+"?page="+(idx_pre*maxlist)+extra+"'>"
	  				+ " &lt;&lt; </a>";
	  	} else {
	  		s_pre = "<span class='list-off'> &lt;&lt; </span>";
	  	}
	  	...
	  	for(int i=0;i<10;i++,idx_start++) {
	  		...
	  		if(idx_start == page)
	  			s_idx = s_idx + "<span class='list-now'> ["+idx_start+"] </span>";
	  		else {
	  			s_idx = s_idx + " <a class='list' href='" + url + "?page=" + idx_start + extra;
	  			s_idx = s_idx + "'> [" + idx_start + "] </a> ";
	  		}
	  	}
		// Next 표시부분
	  	if(idx_start <= totpage ) {
	  		s_next = "<a class='list' href='"+url+"?page="+idx_start+extra+"'>"
	  				+ " &gt;&gt; </a>";
	  	} else {
	  		s_next = "<span class='list-off'> &gt;&gt; </span>";
	  	}
